...
`quad.3form_ab` <- function(M,left,right){ crossprod(crossprod(M, Conj(left)), right) }
`quad.3form_bc` <- function(M,left,right){ cprod(left, (M %*% right)) }
`quad.3form` <- function(M,left,right){
    if(ncol(left) < ncol(right)){
        quad.3form_ab(M,left,right)
    } else {
        quad.3form_bc(M,left,right)
    }
}

`quad.3form.inv` <- function(M,left,right){ drop(cprod(left, solve(M, right))) }

`quad.3tform_ab` <- function(M,left,right){ tcprod(left %*% M,right)}
`quad.3tform_bc` <- function(M,left,right){ tcrossprod(left, tcrossprod(Conj(right), M)) }
`quad.3tform` <- function(M,left,right){
    if(nrow(left) < nrow(right)){
        quad.3tform_ab(M,left,right)
    } else {
        quad.3tform_bc(M,left,right)
    }
}
